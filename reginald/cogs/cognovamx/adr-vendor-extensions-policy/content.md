...
description: "Architecture decision: CogNovaMX' vendor extension namespaces — x-mx- (public) and x-mx-p- (private). Defines implementation-specific fields that extend The Gathering's open standard."
created: 2026-02-17
modified: 2026-02-17
...
**Rejected because:** Adds nesting depth. The existing `mx:` object already contains standard fields (`runbook`, `contentType`). Mixing standard MX metadata with obfuscated private values in the same namespace creates confusion. The `mx:` namespace belongs to The Gathering, not CogNovaMX. The prefix approach keeps everything flat and scannable.
