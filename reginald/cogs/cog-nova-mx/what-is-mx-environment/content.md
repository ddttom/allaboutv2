...
**Stage 1 — Machine boot.** Read `$MX_HOME`. The AI agent loads machine.yaml, repos.yaml, user.yaml. It now knows: this is Tom's MacBook, arm64, macOS. There is one registered repository at this path. Tom Cranstoun is the founder of Cog-Nova-MX. Maxine is the AI partner. The partnership model applies.
