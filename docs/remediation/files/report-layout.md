---
title: "Report Layout Reference for AI Assistants"
description: "Site remediation report: Report Layout Reference for AI Assistants"
...
mx:
  status: active
  canonicalUri: https://raw.githubusercontent.com/ddttom/allaboutv2/main/docs/remediation/files/report-layout.md
---
...
## ⚠️ IMPORTANT: Known Limitations for Adobe Edge Delivery Services (EDS) Sites

The SEO Analysis Tool has **false positive detection issues** when analyzing Adobe EDS sites due to framework-specific optimizations:
...
- **Verification**: Check `/scripts/aem.js` lines 342-357 (createOptimizedPicture function)
...
- ⚠️ **Optimization Score**: May be artificially low due to false negatives above

**Recommendation**: Always verify audit findings against actual implementation before planning fixes, especially for framework-managed features like EDS.
...
- **Recommendation**: Manually verify these fields for EDS sites by:
  1. Checking browser console for lazy loading intervention messages
  2. Inspecting DOM for `<picture>` elements with multiple `<source>` tags
  3. Checking for `loading="lazy"` attributes in image tags
...
## Verification Commands for EDS Sites

When analyzing Adobe Edge Delivery Services sites, use these commands to verify actual implementation vs. audit report findings:
...
# Check aem.js for lazy loading code
curl -s https://allabout.network/scripts/aem.js | grep -i 'loading.*lazy'
```
...
// Should return > 0 if responsive images present

// Check for multiple <source> elements
document.querySelectorAll('picture source').length
// Should return multiple sources per image
```
...
curl -s https://allabout.network/ | grep -c 'srcset='

# Check aem.js for createOptimizedPicture function
curl -s https://allabout.network/scripts/aem.js | grep -A 20 'createOptimizedPicture'
```
...
curl -s https://allabout.network/ | grep -o 'alt="[^"]*"' | head -10
```
...
echo "=== EDS Image Optimization Verification ==="
echo ""

echo "1. Lazy Loading Check:"
echo "   - Check browser console for intervention message"
echo "   - Or verify in aem.js:"
curl -s ${URL}scripts/aem.js | grep -q 'loading.*lazy' && echo "   ✅ Lazy loading code found in aem.js" || echo "   ❌ No lazy loading code"
echo ""

echo "2. Responsive Images Check:"
PICTURE_COUNT=$(curl -s $URL | grep -c '<picture>')
echo "   - Found $PICTURE_COUNT <picture> elements"
[ $PICTURE_COUNT -gt 0 ] && echo "   ✅ Responsive images present" || echo "   ❌ No responsive images"
echo ""

echo "3. Alt Text Check (accurate):"
IMG_NO_ALT=$(curl -s $URL | grep '<img' | grep -v 'alt=' | wc -l)
echo "   - Found $IMG_NO_ALT images without alt text"
[ $IMG_NO_ALT -eq 0 ] && echo "   ✅ All images have alt text" || echo "   ❌ Missing alt text on $IMG_NO_ALT images"
```
...
- **EDS Limitations**: Document updated to reflect known false positives for Adobe EDS sites
